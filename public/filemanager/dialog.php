...
if(!$app['auth']->check()){
	echo 'Bạn chưa đăng nhập/Please login first!';
	die;
}
...
$descending = TRUE; // tuhuge add 07/08/2018
...
if ($aviary_active){
	if (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off' || $_SERVER['SERVER_PORT'] == 443) { ?>
		<script src="https://cdn.ihappy.vn/js/editor.js"></script>
		<?php }else{ ?>
		<script src="http://cdn.ihappy.vn/js/editor.js"></script>
	...
	<div class="navbar navbar-fixed-top">
		<div class="navbar-inner">
			<div class="container-fluid">
					...
					<div class="filters">
						<div class="row-fluid">
							<div class="span6 half">
								<?php if($upload_files){ ?>
								<button class="tip btn upload-btn" title="<?php echo  trans('Upload_file');?>"><i class="rficon-upload"></i> Tải file lên (Upload)</button>
								<?php } ?>
				...
				<button class="tip btn new-folder" title="<?php echo  trans('New_Folder')?>"><i class="icon-folder-open"></i> Tạo thư mục (Create folder)</button>
				<?php } ?>
			...
			<div class="span4 entire types">
				...
				<label id="ff-item-type-5" title="<?php echo trans('Music');?>" for="select-type-5" class="tip btn ff-label-type-5"><i class="icon-music"></i></label>
					<?php } ?>
				<?php } ?>
				<input accesskey="f" type="text" class="filter-input <?php echo (($_GET['type']!=1 && $_GET['type']!=3) ? '' : 'filter-input-notype');?>" id="filter-input" name="filter" placeholder="" value="<?php echo $filter;?>"/><?php if($n_files>$file_number_limit_js){ ?><label id="filter" class="btn"><i class="icon-play"></i></label><?php } ?>
...
<div class="row-fluid">
	...
	<?php } ?> -->
	<li class="pull-right"><a id="refresh" class="btn-small" href="dialog.php?<?php echo $get_params.$subdir."&".uniqid() ?>"><i class="icon-refresh"></i> Làm mới (Refresh)</a></li>
	...
	<li class="pull-right">
		<div class="btn-group">
			...
			<ul class="dropdown-menu pull-left sorting">
				<!-- <li class="text-center"><strong>Sắp xếp</strong></li> -->
				<li><a class="sorter sort-name <?php if($sort_by=="name"){ echo ($descending)?"descending":"ascending"; } ?>" href="javascript:void('')" data-sort="name">Tên file (Filename)</a></li>
				<li><a class="sorter sort-date <?php if($sort_by=="date"){ echo ($descending)?"descending":"ascending"; } ?>" href="javascript:void('')" data-sort="date">Ngày tạo (Date)</a></li>
				<li><a class="sorter sort-size <?php if($sort_by=="size"){ echo ($descending)?"descending":"ascending"; } ?>" href="javascript:void('')" data-sort="size">Kích thước (Size)</a></li>
				<li><a class="sorter sort-extension <?php if($sort_by=="extension"){ echo ($descending)?"descending":"ascending"; } ?>" href="javascript:void('')" data-sort="extension">Loại file (File type)</a></li>
			</ul>
		</div>
	</li>
			...
			if(in_array($file_array['extension'], $ext_img)){
				...
				if($ftp){
					...
					if(!file_exists($src_thumb) ){
						if(!create_img($file_path, $creation_thumb_path, $config['thumb_width'], $config['thumb_height'],'crop',$ftp,$config)){
							$src_thumb=$mini_src="";
						}else{
